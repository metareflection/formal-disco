...
def system_reflect() -> str:
    return (
        ...
        "  NEW_HEURISTIC_INPUT_KINDS: comma-separated concept kinds the heuristic\n"
        "    will operate on. PREFER BROAD selections — most useful heuristics\n"
        "    apply across kinds. Common picks: 'operation,definition' (acts on\n"
        "    primitives), 'definition,theorem' (acts on results), or just 'all'.\n"
        "    Avoid 'theorem' alone unless your template genuinely requires a\n"
        "    proved theorem as input — most agendas have very few theorems\n"
        "    early on, and a theorem-only filter will match nothing.\n"
        "  NEW_HEURISTIC_INPUT_TAGS: comma-separated tag filters. STRONGLY\n"
        "    PREFER LEAVING EMPTY (no tag filter). Tags like 'boundary' or\n"
        "    'arithmetic' are descriptive of the *proved theorems above* but\n"
        "    are usually NOT carried by other concepts the heuristic should\n"
        "    apply to. Empty is the safe default and matches what nearly all\n"
        "    of the existing seed heuristics do.\n"
